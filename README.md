...
project is a hands-on precursor to my independent small-scale thesis project on
...
Access to expert ultrasound review is not evenly distributed — in some
clinical settings, only one specialist may be available to interpret
scans, which can mean longer waits and more room for inconsistent image
quality to affect results. That real-world constraint is part of what
drew me to this specific question, though this project itself is a
small, exploratory step: it does not solve that problem, but it reflects
the kind of technical question — can a model trained in one setting
generalize reliably to another — that would need to be addressed before
AI tools could responsibly assist in such settings.
